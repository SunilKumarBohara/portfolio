  ...
  return (
    <section id="skills" className="relative py-24 sm:py-32 bg-background border-t border-white/5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {skillCategories.map((cat, idx) => {
            ...
            return (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-xs font-mono font-bold transition-all duration-300 flex items-center gap-2 border ${
                  isActive
                    ? idx === 2
                      ? "bg-brand-red/20 text-brand-red border-brand-red/60 shadow-sm"
                      : "bg-brand-blue/20 text-brand-cyan border-brand-blue/60 shadow-sm"
                    : "bg-surface-200/80 text-text-muted border-white/10 hover:text-text-primary hover:border-brand-blue/30"
                }`}
              >
        ...
        >
          {skillCategories[selectedCat].skills.map((skill, idx) => (
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-base font-bold text-text-primary group-hover:text-brand-cyan transition-colors font-display">
                      {skill.name}
                    </h3>
                  </div>
              ...
              {/* Technical Indicator */}
              <div className="mt-6 pt-3 border-t border-white/5 flex items-center justify-between text-[11px] font-mono text-text-muted">
                <span className="flex items-center gap-1.5 text-brand-cyan font-medium">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  Execution Standard
                </span>
                <span className="text-text-muted/80">Core Expertise</span>
              </div>
            </GlassCard>
          ))}
        </motion.div>
        ...
        <div className="mt-16 p-6 rounded-2xl bg-surface-200/50 border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-text-muted font-mono">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-text-primary font-bold">SEO & Web Development Synergy</p>
